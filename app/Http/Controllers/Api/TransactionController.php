...
class TransactionController extends Controller
{
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            return DB::transaction(function () use ($id, $validated) {
                ...
                // Si cambió de no pagado a pagado o cambió el método de pago
                if ((!$oldPaid && $transaction->paid) || ($oldPaymentMethod !== $transaction->payment_method && $transaction->paid)) {
                    // Generar folio específico por método de pago
                    $paymentFolio = $this->generatePaymentMethodFolio(
                ...
                // Si cambió a no pagado, limpiar los folios de método de pago
    ...
    public function getDisplayFolio($transaction)
    {
        ...
        if ($transaction->payment_method === 'card') {
            ...
            $card = \App\Models\Card::find($transaction->card_id);
            if ($card && $card->sat) {
                return $transaction->folio_new; // Usar folio general para tarjetas SAT
            }
            
            // Si la tarjeta no tiene SAT = true, usar folio específico T
            return $transaction->folio_card ? 'T' . str_pad($transaction->folio_card, 4, '0', STR_PAD_LEFT) : $transaction->folio_new;
        }

        // Usar el folio específico del método de pago para otros métodos
        switch ($transaction->payment_method) {
            case 'transfer':
                return $transaction->folio_transfer ? 'A' . str_pad($transaction->folio_transfer, 4, '0', STR_PAD_LEFT) : $transaction->folio_new;
            case 'cash':
                return $transaction->folio_cash ? 'E' . str_pad($transaction->folio_cash, 4, '0', STR_PAD_LEFT) : $transaction->folio_new;
            default:
                return $transaction->folio_new;
        }
    }
